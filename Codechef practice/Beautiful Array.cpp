...
ll solve()
{
    ...
    if(sum%4!=0)
        return -1;
    ll twotwoPair=freq[2]/2;
    ll remainTwo=freq[2]%2;
    remainTwo==0?freq[2]==0:freq[2]==1;
    ll threeOnePair=min(freq[1],freq[3]);
    ll threeOrOneRem=abs(freq[1]-freq[3]);
    if(freq[1]>freq[3])
    {
        freq[3]=0;
        freq[1]=threeOrOneRem;
    }
    else
    {
        freq[1]=0;
        freq[3]=threeOrOneRem;
    }
    if(remainTwo==0)
        return twotwoPair+threeOnePair+(threeOrOneRem/4)*3;
    else
    {
        if(freq[3]==0)
            return twotwoPair+threeOnePair+2+((freq[1]-2)/4)*3;
        else
            return twotwoPair+threeOnePair+2+((freq[3]-2)/4)*3;
    }   
}
