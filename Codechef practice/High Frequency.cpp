...
ll solve()
{
    ...
    for(auto y: fr)
    {
        if(y.first!=mfel && y.second>=secondMaxFreq)
        {
            smfel=y.first;
            secondMaxFreq=y.second;
        }
    }
    if(secondMaxFreq==LLONG_MIN)
        return ceil(maxFreq/2.0);
    if(ceil(maxFreq/2.0)>=secondMaxFreq)
        return ceil(maxFreq/2.0);
