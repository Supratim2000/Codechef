...
ll solve()
{
    ...
    cin>>n;
    cin>>st;
    //keeps track of previous shown charge and position
    pair<char,int> prev('x',-1);
    //keeps track of current charge and position
    pair<char,int> cur;
    ll count=0;
    for(int i=0;i<n;i++)
    {
        if(st[i]!='0')
        {
            cur.first=st[i];
            cur.second=i;
            if(prev.first!='x' && cur.first!=prev.first && cur.second-prev.second!=1 && (cur.second-prev.second)%2==0)
                count++;
            prev.first=cur.first;
            prev.second=cur.second;
        }
    }
    return prev.first=='x'?n:count;
}
